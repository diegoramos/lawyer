...
class Appconfig extends CI_Model 
{
	...
	function exists($key)
	{
		...
		$this->db->where('app_config.name_key',$key);
		$query = $this->db->get();
	...
	{
		$this->db->from('app_config');
		$this->db->where("name_key", "kill_ecommerce_cron");
		$row = $this->db->get()->row_array();
		if (!empty($row))
		{
			return $row['value'];
	...
	{
		$this->db->from('app_config');
		$this->db->where("name_key", "ecommerce_cron_running");
		$row = $this->db->get()->row_array();
		if (!empty($row))
		{
			return $row['value'];
	...
	{
		$this->db->from('app_config');
		$this->db->where("name_key", "number_of_decimals");
		$row = $this->db->get()->row_array();
		if (!empty($row))
		{
			return $row['value'];
	...
	{
		$this->db->from('app_config');
		$this->db->where("name_key", "language");
		$row = $this->db->get()->row_array();
		if (!empty($row))
		{
			return $row['value'];
	...
	{
		$this->db->from('app_config');
		$this->db->where("name_key", "version");
		$row = $this->db->get()->row_array();
		if (!empty($row))
		{
			return $row['value'];
	...
	function get_force_https()
	{
		...
		{
			$this->db->from('app_config');
			$this->db->where("name_key", "force_https");
			$row = $this->db->get()->row_array();
			if (!empty($row))
			{
				return $row['value'];
	...
	{
     $this->db->from('app_config');
     $this->db->where("name_key", "do_not_force_http");
     $row = $this->db->get()->row_array();
     if (!empty($row))
     {
			 return $row['value'];
	...
	{
		$this->db->from('app_config');
		$this->db->where("name_key", "phppos_session_expiration");
		$row = $this->db->get()->row_array();
		if (!empty($row))
		{
	...
	{
		$this->db->query('REPLACE INTO '.$this->db->dbprefix('app_config').' (`name_key`, `value`) VALUES ("mercury_activate_seen", "'.($mercury_activate_seen ? 1 : 0).'")');
	}
